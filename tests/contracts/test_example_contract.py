...
def test_committed_example_validates_against_response_model() -> None:
    """커밋된 예시 JSON은 v0.6 Pydantic 응답 계약을 통과해야 한다."""

    payload = json.loads((ROOT / "docs/examples/v0.6/synthetic/recommend.output.json").read_text())
    response = DayTripResponse.model_validate(payload)
    assert response.schema_version == "0.6.0"
